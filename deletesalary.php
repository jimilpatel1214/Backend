...
<form method="post">
  <table>
    <tr>
      ...
      <th width="250px">bank account</th>
      <th width="250px">Salary</th>
    ...
    while ($row = $sql->fetch_assoc()){
      ...
      <tr>
        ...
        <td>{$row['Salary']}</td>
        <td><input type='checkbox' name='salary_ids[]' value='{$row['teacherid']}'></td>
      </tr>";
    }
    ?>
    ...
    <tr>
      <td colspan="8"><input type="submit" name="delete" value="Delete Selected salary"></td>
    </tr>
  </table>
</form>
...
if(isset($_POST['delete'])) {
    ...
    $_ids = $_POST['salary_ids'];

    $salary_ids_str = implode(",", $_ids);

    $sql = "DELETE FROM salary WHERE teacherid IN ($salary_ids_str)";
    if(mysqli_query($link, $sql)) {
        echo "Salary records deleted successfully.";
    } else {
        echo "Error deleting salary records: " . mysqli_error($link);
    }
}
?>
