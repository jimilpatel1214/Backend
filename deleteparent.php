...
$sql = mysqli_query($link, "SELECT `parentsid`, `parentsname`, `parentscontact`, `email` , `enrolmentid` FROM `parents`");
...
<style>
    ...
    <h3>Delete Parents Records</h3>

    <form method="post">
      <table>
        <tr>
          <th width="150px">parents id</th>
          <th width="250px">parents name</th>
          <th width="250px">parents contact</th>
        ...
        while ($row = $sql->fetch_assoc()){
          ...
          <tr>
            ...
            <td><input type='checkbox' name='parent_ids[]' value='{$row['parentsid']}'></td>
          </tr>";
        }
        ?>

        <tr>
          <td colspan="8"><input type="submit" name="delete" value="Delete Selected parents"></td>
        </tr>
      </table>
    </form>
    <?php
...
if(isset($_POST['delete'])) {
    ...
    $sql = "DELETE FROM parents WHERE parentsid IN ($parent_ids_str)";
    if(mysqli_query($link, $sql)) {
      echo "Parents records deleted successfully.";
    } else {
      echo "Error deleting Parents records: " . mysqli_error($link);
    }
  }
?>  
